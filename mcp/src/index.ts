...
server.tool(
  ...
  "List all available genome assemblies in ChIP-Atlas (e.g. hg38, mm10, dm6)",
  {},
  async () => {
...
server.tool(
  "chipatlas_list_experiment_types",
  "List experiment types (e.g. Histone, TFs and others, ATAC-Seq). Optionally provide genome and cell type class to get counts.",
  {
    genome: z
      .string()
      .optional()
...
server.tool(
  "chipatlas_list_sample_types",
  "List cell type classes with experiment counts for a given genome and experiment type.",
  {
...
server.tool(
  ...
  "List antigens/subclasses (e.g. H3K4me3, CTCF) with counts for a given genome and experiment class.",
  {
    genome: z.string().describe("Genome assembly (e.g. hg38, mm10)"),
    agClass: z
      .string()
...
server.tool(
  "chipatlas_list_cell_types",
  "List cell type subclasses with counts for a given genome and experiment class.",
  {
    genome: z.string().describe("Genome assembly (e.g. hg38, mm10)"),
    agClass: z
      .string()
...
server.tool(
  "chipatlas_search_experiments",
  "Search ChIP-Atlas experiments by keyword. Searches across all fields (SRX ID, GEO ID, genome, antigen, cell type, etc.). Returns up to `limit` results.",
  {
    query: z
      .string()
  ...
  async ({ query, limit }) => {
    const results = await client.searchExperiments(query, limit ?? 20);
...
server.tool(
  ...
  "Get detailed metadata for a single experiment by its SRX or GSM ID.",
  {
...
server.tool(
  ...
  "Get colocalization analysis data for a genome. Returns available antigens and cell types that have colocalization data.",
  {
    genome: z.string().describe("Genome assembly (e.g. hg38, mm10)"),
  },
  async ({ genome }) => {
...
server.tool(
  ...
  "Get target genes analysis availability. Returns a mapping of genome assemblies to lists of antigens that have target gene analysis data.",
  {},
  async () => {
...
server.tool(
  ...
  "Get a download URL for peak-call BED data given filtering conditions.",
  {
    genome: z.string().describe("Genome assembly (e.g. hg38, mm10)"),
    agClass: z
      .string()
