...
function run_enrichment-analysis() {
  local ea_job=${exe_dir}/$(basename ${run_dir}).json
  ...
  local bedAFile_path=${exe_dir}/bedAFile_$(basename ${run_dir}).bed
  local bedBFile_path=${exe_dir}/bedBFile_$(basename ${run_dir}).bed
